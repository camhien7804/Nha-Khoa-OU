...
export default function BookingInline({ open = true, onClose }) {
  ...
  const [form, setForm] = useState({
    ...
    note: "",
    paymentMethod: "cash",
    selectedPrice: "", // ✅ luôn là GIÁ GỐC (minPrice/maxPrice), không phải giá sau giảm
  });
  ...
  const validate = () => {
    ...
    // Nếu dịch vụ có khoảng giá phải chọn selectedPrice (raw)
    const svc = services.find(
      (s) => s._id === form.service || s.slug === form.service || s.name === form.service
    );
    if (svc && svc.minPrice !== svc.maxPrice && !form.selectedPrice) {
      return "Vui lòng chọn giá cho dịch vụ.";
    }
    return null;
  };
  ...
  const onSubmit = async (e) => {
    ...
    try {
      ...
      const body = {
        ...
        // ✅ gửi GIÁ GỐC nếu có range (server sẽ tự giảm một lần)
        selectedPrice:
          found && found.minPrice !== found.maxPrice
      ...
      if (res?.data?.success) {
        ...
        // MoMo: ví, ATM, CC, ví trả sau → đều qua cổng MoMo nhưng khác requestType
        if (["momo", "atm", "cc", "vts"].includes(form.paymentMethod)) {
          try {
            const payRes = await api.post("/payment/momo/create", {
            ...
            alert(payRes.data.message || "Không tạo được thanh toán");
            return;
          } catch (err) {
  ...
  // Helper render giá hiển thị (đã giảm nếu có), nhưng value option là GIÁ GỐC
  const renderPriceLabel = (raw, discountPercent) => {
    const discounted =
      discountPercent > 0 ? Math.round(raw * (1 - discountPercent / 100)) : raw;
    return discountPercent > 0
      ? `${discounted.toLocaleString()} đ (gốc ${raw.toLocaleString()} đ, -${discountPercent}%)`
      : `${raw.toLocaleString()} đ`;
  };

  return (
    <div className="fixed inset-0 z-[100] flex items-center justify-center">
      <div className="absolute inset-0 bg-black/40" onClick={onClose} />
      <div className="relative bg-white w-full max-w-lg mx-auto rounded-2xl shadow-lg p-6 animate-fadeIn">
        ...
        <form onSubmit={onSubmit} className="mt-4 space-y-3">
            ...
            {!loadingServices && services.length > 0 ? (
              <select
                ...
                onChange={(e) => {
                  ...
                  // Nếu là fixed price, có thể set sẵn selectedPrice = GIÁ GỐC
                  setForm((f) => ({
                    ...f,
                    service: s?._id || val,
                    serviceName: s?.name || s?.title || val,
                    selectedPrice:
                      s && s.minPrice === s.maxPrice ? String(s.minPrice) : "",
                  }));
                }}
                className="w-full border rounded px-3 py-2 focus:ring-2 focus:ring-green-500 transition"
              >
          ...
          {/* Chọn giá khi có min/max (value = GIÁ GỐC) */}
          {form.service &&
            (() => {
              const svc = services.find(
                (s) =>
              ...
              if (svc && svc.minPrice !== svc.maxPrice) {
                return (
                  <div>
                    <label className="block mb-1">Chọn giá *</label>
                    <select
                      name="selectedPrice"
                    ...
                    >
                      ...
                      <option value={svc.minPrice}>
                        {renderPriceLabel(svc.minPrice, svc.discountPercent)}
                      </option>
                      <option value={svc.maxPrice}>
                        {renderPriceLabel(svc.maxPrice, svc.discountPercent)}
                      </option>
                    </select>
                  </div>
                );
          ...
          <div>
            ...
            >
              ...
              <option value="cc">Visa/MasterCard/JCB</option>
              <option value="vts">Ví trả sau MoMo</option>
            </select>
          </div>
