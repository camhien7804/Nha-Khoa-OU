...
export default function Register() {
  ...
  const submit = async (e) => {
    ...
    if (!f.fullName || !f.email || !f.password) {
      return alert(t("Vui lòng nhập đầy đủ thông tin bắt buộc"));
    }
    if (f.password !== f.confirm) {
      return alert(t("Mật khẩu nhập lại không khớp"));
    }
    try {
      setLoading(true);
      await register(f);
      ...
      <div className="w-full max-w-2xl bg-white rounded-xl shadow-sm p-6">
        ...
        <form onSubmit={submit} className="grid md:grid-cols-2 gap-4">
          {[
            ...
            ["dob", t("Ngày sinh *"), "date"],
            ["email", t("Email *"), "email"],
            ["password", t("Mật khẩu *"), "password"],
            ["confirm", t("Xác nhận mật khẩu *"), "password"],
            ["address", t("Địa chỉ")],
          ].map(([k, label, type]) => (
          ...
          <div className="md:col-span-1">
            ...
            >
              ...
              <option value="Nữ">{t("Nữ")}</option>
              <option value="Khác">{t("Khác")}</option>
            </select>
          </div>
          ...
          <div className="md:col-span-2">
            ...
            >
              {loading ? t("Đang xử lý…") : t("Tạo tài khoản")}
            </button>
          </div>
        </form>
      </div>
